...
public:
  ...
  llvm::Value *getPointerFromCapability(CodeGen::CodeGenFunction &CGF,
    ...
    auto &B = CGF.Builder;
    assert(VTy->isPointerTy() && VTy->getPointerAddressSpace() == AS);
    V = B.CreateBitCast(V, getI8CapTy(CGF));
    auto DDC =
        CGF.Builder.CreateIntrinsic(llvm::Intrinsic::cheri_ddc_get, {}, {});
    V = B.CreateCall(CGF.CGM.getIntrinsic(llvm::Intrinsic::cheri_cap_to_pointer,
                                          CGF.IntPtrTy),
                     {DDC, V});
...
class AArch64TargetCodeGenInfo : public CommonCheriTargetCodeGenInfo,
    CHERICapClassifier {
public:
