...
// RUN: %cheri_cc1 %s -o - -emit-llvm -O1 -Weverything -Werror -verify -Wno-declaration-after-statement | FileCheck %s --check-prefixes=CHECK,HYBRID
// RUN: %cheri_purecap_cc1 %s -o - -emit-llvm -O1 -Weverything -Werror -Wno-declaration-after-statement -verify | FileCheck %s --check-prefixes=CHECK,PURECAP
...
// HYBRID-LABEL: @test_loadtags(
// HYBRID-NEXT:  entry:
// HYBRID-NEXT:    [[TMP0:%.*]] = tail call i64 @llvm.cheri.cap.load.tags.i64.p0(ptr [[PTR:%.*]])
// HYBRID-NEXT:    tail call void @use_size_t(i64 noundef zeroext [[TMP0]]) #[[ATTR5]]
// HYBRID-NEXT:    ret void
//
// PURECAP-LABEL: @test_loadtags(
// PURECAP-NEXT:  entry:
// PURECAP-NEXT:    [[TMP0:%.*]] = tail call i64 @llvm.cheri.cap.load.tags.i64.p200(ptr addrspace(200) [[PTR:%.*]])
// PURECAP-NEXT:    tail call void @use_size_t(i64 noundef zeroext [[TMP0]]) #[[ATTR5]]
