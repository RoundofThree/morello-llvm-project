...
void AArch64FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  emitFrameOffset(
      MBB, CalleeSavesBegin, DL, SP, SP, -AllocateBefore, TII,
...
void AArch64FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  if (CombineSPBump) {
    ...
    if (EmitCFI && hasFP(MF)) {
      const AArch64RegisterInfo &RegInfo = *Subtarget.getRegisterInfo();
      unsigned Reg = RegInfo.getDwarfRegNum(SP, true);
      unsigned CFIIndex =
  ...
  if (AfterCSRPopSize) {
    ...
    emitFrameOffset(
        MBB, MBB.getFirstTerminator(), DL, SP, SP,
