...
// Test expansion of fat pointer expressions. These should avoid inttoptr
// on anything except constant expressions.
// FIXME: At some point fat pointers should be a type of non-integral pointer,
// but there are actually more constraints here. These are maintaining
// provenance and not taking the pointer too much outside the bounds. Solving
// the provenance issue likely requires new expression types, so everything here
// will probably change anyway at some point.
TEST_F(ScalarEvolutionExpanderTest, SCEVExpandFatPointer) {
  // Create a module with non-integral pointers in it's datalayout
  ...
  auto *SMax = SE.getSMaxExpr(SE.getUnknown(Val), SE.getSCEV(Arg1));
  Exp.expandCodeFor(SMax, T_pint64, Ret);
  auto *UMax = SE.getUMaxExpr(SE.getUnknown(Val), SE.getSCEV(Arg1));
