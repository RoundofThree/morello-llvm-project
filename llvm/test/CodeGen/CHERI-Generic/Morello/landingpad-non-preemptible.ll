...
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 < %s -o - | FileCheck %s --check-prefixes=CHECK,NOCODEPTR
; RUN: llc -mtriple=aarch64 --relocation-model=pic -target-abi purecap -mattr=+morello,+c64 < %s -o - -cheri-codeptr-relocs | FileCheck %s --check-prefixes=CHECK,CODEPTR
...
; NOCODEPTR-NEXT:  .chericap  .L_Z8do_catchv$eh_alias+(.Ltmp2-.Lfunc_begin0)   {{#|//}}     jumps to .Ltmp2
; CODEPTR-NEXT:  .chericap  .L_Z8do_catchv$eh_alias@code+(.Ltmp2-.Lfunc_begin0)   {{#|//}}     jumps to .Ltmp2
