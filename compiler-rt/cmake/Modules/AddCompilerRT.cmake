...
macro(set_output_name output name arch)
  if(LLVM_ENABLE_PER_TARGET_RUNTIME_DIR)
    ...
    elseif(${arch} STREQUAL "aarch64")
      # Distinguish between CHERI purecap ABIs
      set(LAST_MABI_VALUE "aapcs")
      string(REGEX MATCHALL "-mabi=[^ ]+" MABI_FLAGS "${CMAKE_C_FLAGS}")
      if(MABI_FLAGS)
        list(REVERSE MABI_FLAGS)
        string(REGEX MATCH "-mabi=([^ ]+)" LAST_MABI_FLAG "${MABI_FLAGS}")
        string(REGEX REPLACE "-mabi=(.*)" "\\1" LAST_MABI_VALUE "${LAST_MABI_FLAG}")
      endif()
      if(${LAST_MABI_VALUE} STREQUAL "aapcs")
        set(${output} "${name}-${arch}${COMPILER_RT_OS_SUFFIX}")
      elseif(${LAST_MABI_VALUE} STREQUAL "purecap")
        set(${output} "${name}-${arch}c${COMPILER_RT_OS_SUFFIX}")
      elseif(${LAST_MABI_VALUE} STREQUAL "purecap-benchmark")
        set(${output} "${name}-${arch}cb${COMPILER_RT_OS_SUFFIX}")
      else()
        message(FATAL_ERROR "Unrecognised ABI")
      endif()
    else()
      set(${output} "${name}-${arch}${COMPILER_RT_OS_SUFFIX}")
    endif()
