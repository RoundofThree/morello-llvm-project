...
class DeadlockDetectorTLS {
 public:
  ...
  void ensureCurrentEpoch(usize current_epoch) {
  ...
  usize getEpoch() const { return epoch_; }

  // Returns true if this is the first (non-recursive) acquisition of this lock.
  bool addLock(usize lock_id, usize current_epoch, u32 stk) {
  ...
  u32 findLockContext(usize lock_id) {
    for (usize i = 0; i < n_all_locks_; i++)
      if (all_locks_with_contexts_[i].lock == static_cast<u32>(lock_id))
        return all_locks_with_contexts_[i].stk;
    return 0;
  }

  const BV &getLocks(usize current_epoch) const {
    CHECK_EQ(epoch_, current_epoch);
    return bv_;
  }

  usize getNumLocks() const { return n_all_locks_; }
  usize getLock(usize idx) const { return all_locks_with_contexts_[idx].lock; }

 private:
  BV bv_;
  usize epoch_;
  usize recursive_locks[64];
  usize n_recursive_locks;
...
class DeadlockDetector {
 public:
  ...
  uptr getData(usize node) const { return data_[nodeToIndex(node)]; }

  bool nodeBelongsToCurrentEpoch(usize node) {
    return node && (node / size() * size()) == current_epoch_;
  }

  void removeNode(usize node) {
    usize idx = nodeToIndex(node);
  ...
  bool onLockBefore(DeadlockDetectorTLS<BV> *dtls, usize cur_node) {
    ensureCurrentEpoch(dtls);
    usize cur_idx = nodeToIndex(cur_node);
    return g_.isReachable(cur_idx, dtls->getLocks(current_epoch_));
  }

  u32 findLockContext(DeadlockDetectorTLS<BV> *dtls, usize node) {
    return dtls->findLockContext(nodeToIndex(node));
  }

  // Add cur_node to the set of locks held currently by dtls.
  void onLockAfter(DeadlockDetectorTLS<BV> *dtls, usize cur_node, u32 stk = 0) {
    ensureCurrentEpoch(dtls);
    usize cur_idx = nodeToIndex(cur_node);
  ...
  bool hasAllEdges(DeadlockDetectorTLS<BV> *dtls, usize cur_node) {
    usize local_epoch = dtls->getEpoch();
    // Read from current_epoch_ is racy.
    if (cur_node && local_epoch == current_epoch_ &&
        local_epoch == nodeToEpoch(cur_node)) {
      usize cur_idx = nodeToIndexUnchecked(cur_node);
      for (usize i = 0, n = dtls->getNumLocks(); i < n; i++) {
  ...
  usize addEdges(DeadlockDetectorTLS<BV> *dtls, usize cur_node, u32 stk,
                int unique_tid) {
    ensureCurrentEpoch(dtls);
    usize cur_idx = nodeToIndex(cur_node);
    usize added_edges[40];
    usize n_added_edges = g_.addEdges(dtls->getLocks(current_epoch_), cur_idx,
                                     added_edges, ARRAY_SIZE(added_edges));
    for (usize i = 0; i < n_added_edges; i++) {
  ...
  bool findEdge(usize from_node, usize to_node, u32 *stk_from, u32 *stk_to,
                int *unique_tid) {
    usize from_idx = nodeToIndex(from_node);
    usize to_idx = nodeToIndex(to_node);
    for (usize i = 0; i < n_edges_; i++) {
  ...
  bool onLock(DeadlockDetectorTLS<BV> *dtls, usize cur_node, u32 stk = 0) {
    ensureCurrentEpoch(dtls);
  ...
  bool onTryLock(DeadlockDetectorTLS<BV> *dtls, usize cur_node, u32 stk = 0) {
    ensureCurrentEpoch(dtls);
    usize cur_idx = nodeToIndex(cur_node);
    dtls->addLock(cur_idx, current_epoch_, stk);
    return false;
  ...
  bool onFirstLock(DeadlockDetectorTLS<BV> *dtls, usize node, u32 stk = 0) {
  ...
  // or 0 on failure.
  usize findPathToLock(DeadlockDetectorTLS<BV> *dtls, usize cur_node, usize *path,
                      usize path_size) {
    tmp_bv_.copyFrom(dtls->getLocks(current_epoch_));
    usize idx = nodeToIndex(cur_node);
    CHECK(!tmp_bv_.getBit(idx));
    usize res = g_.findShortestPath(idx, tmp_bv_, path, path_size);
    for (usize i = 0; i < res; i++)
  ...
  // This operation is thread-safe as it only touches the dtls.
  void onUnlock(DeadlockDetectorTLS<BV> *dtls, usize node) {
  ...
  bool onLockFast(DeadlockDetectorTLS<BV> *dtls, usize node, u32 stk = 0) {
  ...
  bool isHeld(DeadlockDetectorTLS<BV> *dtls, usize node) const {
    return dtls->getLocks(current_epoch_).getBit(nodeToIndex(node));
  }

  usize testOnlyGetEpoch() const { return current_epoch_; }
  bool testOnlyHasEdge(usize l1, usize l2) {
  ...
  void Print() {
    for (usize from = 0; from < size(); from++)
      for (usize to = 0; to < size(); to++)
        if (g_.hasEdge(from, to))
          Printf("  %zx => %zx\n", from, to);
  }

 private:
  void check_idx(usize idx) const { CHECK_LT(idx, size()); }

  void check_node(usize node) const {
    CHECK_GE(node, size());
    CHECK_EQ(current_epoch_, nodeToEpoch(node));
  }

  usize indexToNode(usize idx) const {
    check_idx(idx);
    return idx + current_epoch_;
  }

  usize nodeToIndexUnchecked(usize node) const { return node % size(); }

  usize nodeToIndex(usize node) const {
    check_node(node);
    return nodeToIndexUnchecked(node);
  }

  usize nodeToEpoch(usize node) const { return node / size() * size(); }

  usize getAvailableNode(uptr data) {
    usize idx = available_nodes_.getAndClearFirstOne();
    data_[idx] = data;
