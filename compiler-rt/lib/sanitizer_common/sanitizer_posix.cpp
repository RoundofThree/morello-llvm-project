...
void *MapFileToMemory(const char *file_name, usize *buff_size) {
  ...
  *buff_size = RoundUpTo(fsize, GetPageSizeCached());
  ...
  // CHERI alignment
  *buff_size = __builtin_cheri_round_representable_length(*buff_size);
#endif
...
void *MapWritableFileToMemory(void *addr, usize size, fd_t fd, OFF_T offset) {
  ...
  addr = __builtin_align_up(addr, ~__builtin_cheri_representable_alignment_mask(size) + 1);
#endif
...
#if SANITIZER_FREEBSD && defined(__aarch64__) && __has_feature(capabilities)
  // CHERI alignment
  length = __builtin_cheri_round_representable_length(length);
  addr = __builtin_align_up(addr, ~__builtin_cheri_representable_alignment_mask(length) + 1);
#endif
