...
// }
// XXXR3: we can convert addr to vaddr type instead of uptr
...
class AddrHashMap {
 private:
  ...
  static const usize kBucketSize = 3;
...
void AddrHashMap<T, kSize>::ForEach(ForEachCallback cb, void *arg) {
  for (usize n = 0; n < kSize; n++) {
    Bucket *bucket = &table_[n];

    ReadLock lock(&bucket->mtx);

    for (usize i = 0; i < kBucketSize; i++) {
...
void AddrHashMap<T, kSize>::acquire(Handle *h)
 ...
 locked:
  ...
  if (add->size == add->cap) {
    ...
    usize oldsize = sizeof(*add) + (add->cap - 1) * sizeof(add->cells[0]);
    usize newsize = oldsize * 2;
