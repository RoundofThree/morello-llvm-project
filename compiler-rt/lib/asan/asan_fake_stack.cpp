...
ALWAYS_INLINE void SetShadow(uptr ptr, usize size, usize class_id, u64 magic) {
  u64 *shadow = reinterpret_cast<u64*>(MemToShadow(ptr));
  if (ASAN_SHADOW_SCALE == 3 && class_id <= 6) {
    // This code expects ASAN_SHADOW_SCALE=3.
    for (usize i = 0; i < (((usize)1) << class_id); i++) {
      shadow[i] = magic;
...
void FakeStack::Destroy(int tid) {
  ...
  if (Verbosity() >= 2) {
    ...
    for (usize class_id = 0; class_id < kNumberOfSizeClasses; class_id++)
      str.append("%zd: %zd/%zd; ", class_id, hint_position_[class_id],
                 NumberOfFrames(stack_size_log(), class_id));
    Report("T%d: FakeStack destroyed: %s\n", tid, str.data());
  }
  usize size = RequiredSize(stack_size_log_);
...
FakeFrame *FakeStack::Allocate(usize stack_size_log, usize class_id,
  ...
  if (needs_gc_)
    GC(real_stack);
  usize &hint_position = hint_position_[class_id];
  const int num_iter = NumberOfFrames(stack_size_log, class_id);
  u8 *flags = GetFlags(stack_size_log, class_id);
  for (int i = 0; i < num_iter; i++) {
    usize pos = ModuloNumberOfFrames(stack_size_log, class_id, hint_position++);
...
uptr FakeStack::AddrIsInFakeStack(uptr ptr, uptr *frame_beg, uptr *frame_end) {
  usize stack_size_log = this->stack_size_log();
  uptr beg = reinterpret_cast<uptr>(GetFrame(stack_size_log, 0, 0));
  uptr end = reinterpret_cast<uptr>(this) + RequiredSize(stack_size_log);
  if (ptr < beg || ptr >= end) return 0;
  usize class_id = (ptr - beg) >> stack_size_log;
  uptr base = beg + (class_id << stack_size_log);
  CHECK_LE(base, ptr);
  CHECK_LT(ptr, base + (((usize)1) << stack_size_log));
  usize pos = (usize)(ptr - base) >> (kMinStackFrameSizeLog + class_id);
...
NOINLINE void FakeStack::GC(uptr real_stack) {
  ...
  for (usize class_id = 0; class_id < kNumberOfSizeClasses; class_id++) {
    u8 *flags = GetFlags(stack_size_log(), class_id);
    for (usize i = 0, n = NumberOfFrames(stack_size_log(), class_id); i < n;
         i++) {
...
static ALWAYS_INLINE uptr OnMalloc(usize class_id, usize size) {
...
static ALWAYS_INLINE uptr OnMallocAlways(usize class_id, usize size) {
...
static ALWAYS_INLINE void OnFree(uptr ptr, usize class_id, usize size) {
...
#define DEFINE_STACK_MALLOC_FREE_WITH_CLASS_ID(class_id)                      \
  ...
  extern "C" SANITIZER_INTERFACE_ATTRIBUTE uptr                               \
      __asan_stack_malloc_always_##class_id(usize size) {                      \
