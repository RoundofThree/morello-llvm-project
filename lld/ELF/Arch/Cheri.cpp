...
void MorelloCapRelocsSection::addCapReloc(CheriCapRelocLocation loc,
  ...
  addEntry(loc, {target, capabilityOffset, targetNeedsDynReloc});
}
...
void MorelloCapRelocsSection::finalizeContents() {
  ...
  if (auto *r = symtab->find("__cap_relocs_end"))
    if (auto *d = dyn_cast<Defined>(r))
...
static bool alignToRequired(OutputSection *first, OutputSection *last,
                            uint64_t reqdAlign) {
  bool changed = false;
  if (reqdAlign == 1)
    return changed;
  if (reqdAlign > first->alignment) {
    first->alignment = reqdAlign;
    changed = true;
  }
