...
RegisterContextUnwind::RegisterContextUnwind(Thread &thread,
    ...
    : RegisterContext(thread, frame_number), m_thread(thread),
      ...
      m_current_offset_backed_up_one(0), m_behaves_like_zeroth_frame(false),
      m_sym_ctx(sym_ctx),
...
void RegisterContextUnwind::InitializeZerothFrame() {
  ...
  if (lang_runtime_plan_sp.get()) {
    ...
    if (!ReadFrameAddress(row_register_kind, active_row->GetCFAValue(),
      ...
      UnwindLogMsg("m_cfa = 0x%" PRIx64 " m_afa = 0x%" PRIx64,
                   m_cfa.GetAddress(), m_afa.GetAddress());
      UnwindLogMsg(
          "initialized async frame current pc is 0x%" PRIx64
          " cfa is 0x%" PRIx64 " afa is 0x%" PRIx64,
          (uint64_t)m_current_pc.GetLoadAddress(exe_ctx.GetTargetPtr()),
          (uint64_t)m_cfa.GetAddress(), (uint64_t)m_afa.GetAddress());
...
void RegisterContextUnwind::InitializeNonZerothFrame() {
  ...
  if (lang_runtime_plan_sp.get()) {
    ...
    if (!ReadFrameAddress(row_register_kind, active_row->GetCFAValue(),
      ...
      UnwindLogMsg("m_cfa = 0x%" PRIx64 " m_afa = 0x%" PRIx64, m_cfa.GetAddress(),
                   m_afa.GetAddress());
      UnwindLogMsg(
          "initialized async frame current pc is 0x%" PRIx64
          " cfa is 0x%" PRIx64 " afa is 0x%" PRIx64,
...
bool RegisterContextUnwind::GetSavedLocationForRegisterThroughAlias(
...
bool RegisterContextUnwind::GetCFA(FrameAddressLLDB &cfa) {
  if (!IsValid()) {
    return false;
  }
